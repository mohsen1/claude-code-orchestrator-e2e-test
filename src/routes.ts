...
router.post('/api/notes', (req: Request, res: Response) => {
  try {
    ...
    const note = storage.createNote({ title, content, tags });
...
// GET /api/notes - List all notes with optional tag and search filters
...
// GET /api/notes/:id - Get a single note with rendered HTML
...
router.put('/api/notes/:id', (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    const updatedNote = storage.updateNote(id, { title, content, tags });
...
// GET /api/tags - Get all unique tags with note counts
