...
// GET /api/groups/[id]/expenses - List all expenses for a specific group
  ...
  try {
    ...
    for (const expense of expenses) {
      ...
      expense.paid_by_user = {
        id: expense.paid_by,
        name: (expense as any).paid_by_name,
        email: (expense as any).paid_by_email,
      };
...
// POST /api/groups/[id]/expenses - Create a new expense in a specific group
export async function POST(
  ...
  try {
    ...
    const { description, amount, split_with } = body;
    ...
    if (split_with && split_with.length > 0) {
      ...
      const invalidUsers = split_with.filter((userId) => !validUsers.has(userId));
    ...
    ).run(expenseId, id, description.trim(), amount, session.user.id);
